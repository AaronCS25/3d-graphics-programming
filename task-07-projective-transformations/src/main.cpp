...
// Camera: one full orbit in 18 seconds, always looking at the origin.
constexpr float kFieldOfView = 45.0f; // degrees
constexpr float kNearPlane = 0.1f;
constexpr float kFarPlane = 100.0f;
constexpr float kCameraRadius = 10.0f; // fits the sphere and moon throughout the orbit
constexpr float kCameraPeriod = 18.0f; // seconds
constexpr float kCameraHeight = 3.5f;
constexpr float kCameraHeightSwing = 1.2f;
...
void updateTitle(GLFWwindow* window) {
    ...
    std::snprintf(title, sizeof(title), "Task 07 - Camera | %s | camera %s | speed %.2fx",
                  gPaused ? "paused" : "running", gCameraMoving ? "orbiting" : "fixed",
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Move the eye in world space; lookAt expresses the scene in its frame.
        // A separate clock lets C freeze the camera while the objects keep moving.
        const float cameraAngle = glm::two_pi<float>() * cameraTime / kCameraPeriod;
        const glm::vec3 cameraPosition{
            kCameraRadius * std::sin(cameraAngle),
            kCameraHeight + kCameraHeightSwing * std::sin(cameraAngle),
            kCameraRadius * std::cos(cameraAngle)};
        const glm::mat4 view = glm::lookAt(cameraPosition, kCameraTarget, kCameraUp);
