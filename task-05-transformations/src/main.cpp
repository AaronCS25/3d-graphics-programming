...
// sphere. The camera never moves: every motion in the scene is a model matrix
// rebuilt from the elapsed time, which is what the statement asks for.
//
// The whole task is the order of the products. A matrix chain reads right to
// left, and each factor acts on the frame the factors to its right have already
// established:
//
//   sphere = orbitPlane * rotateY(orbit) * translate(R) * rotateY(spin) * scale(s)
//            \___ where the sphere is ___/ \___ what it does once there ___/
//
// Swapping translate(R) and rotateY(orbit) would spin the sphere in place at
// distance R instead of carrying it around the cube — the same two matrices,
// a completely different animation.
//
// Same skeleton as tasks 01 and 02, plus what 3D needs: a depth buffer, and the
// model / view / projection chain in the vertex shader.
...
// Camera: fixed, looking at the origin from slightly above.
constexpr float kFieldOfView = 45.0f; // degrees
constexpr float kNearPlane = 0.1f;
constexpr float kFarPlane = 100.0f;
const glm::vec3 kCameraPosition{0.0f, 2.4f, 6.6f};
const glm::vec3 kCameraTarget{0.0f, 0.0f, 0.0f};
const glm::vec3 kCameraUp{0.0f, 1.0f, 0.0f};
...
void main() {
    ...
    FragColor = vec4(mix(vColor, shaded, uLit), 1.0);
}
)";
...
void updateTitle(GLFWwindow* window) {
    char title[128];
    std::snprintf(title, sizeof(title), "Task 05 - Animated Scene | %s | speed %.2fx",
                  gPaused ? "paused" : "running", static_cast<double>(gSpeed));
...
int main() {
    ...
    GLFWwindow* window =
        glfwCreateWindow(kWindowWidth, kWindowHeight, "Task 05 - Animated Scene", nullptr, nullptr);
    if (window == nullptr) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        const glm::mat4 view = glm::lookAt(kCameraPosition, kCameraTarget, kCameraUp);
        const glm::mat4 projection =
            glm::perspective(glm::radians(kFieldOfView), aspect, kNearPlane, kFarPlane);
