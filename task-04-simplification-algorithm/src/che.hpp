...
class CHE {
  public:
    ...
    // SIGGRAPH 1997: iterated edge collapse, cheapest edge first, where the cost
    // of placing a vertex at v is v^T Q v, the sum of squared distances to the
    // planes of the faces that originally met there.

    struct SimplifyReport {
        int collapses = 0;
        int rejected_link = 0;   // would have pinched the surface
        int rejected_flip = 0;   // would have inverted a face
        double total_cost = 0.0; // summed quadric error of every collapse
        double max_cost = 0.0;   // worst single collapse
    };

    // Collapses edges until at most `target_triangles` remain, then rebuilds G,
    // V and O compactly so the result is a valid L1 CHE again.
    SimplifyReport simplify(int target_triangles);
    ...
    // Sampled Hausdorff distance to `reference`, measured in both directions:
    // vertices of each mesh to the closest point on the surface of the other.
    // At most `max_samples` vertices per direction, so large meshes stay usable.
    Deviation deviation_from(const CHE& reference, int max_samples = 2000) const;
  ...
  private:
    ...
    // Collapsing this edge would pinch the surface: the endpoints share a
    // neighbour other than the two vertices opposite the edge.
    bool link_condition(int he) const;

    // Collapsing to `target` would turn a surviving face inside out.
    bool would_flip(int he, const glm::vec3& target) const;

    // Rebuilds G, V and O without the dead triangles and the vertices only they
    // used, remapping every index.
    void compact(const std::vector<bool>& dead_triangle);
